...
const Table = () => {
  const { itemsData, editingItemsData, setField, addItem, removeItem } =
    useItemsStore();
  ...
  return (
    <>
        ...
        <thead>
          <tr className="bg-[#1e3a8a] text-[#ffffff]">
            {tableConfig.map((col, i) => (
              <th
                key={i}
                className={`py-2 px-2 text-left ${col.width} capitalize max-lg:text-[12px]`}
              >
                {col.label}
              </th>
            ))}
      ...
      <div className="button mt-4">
        <Button
          className="px-4 py-2 rounded-lg bg-[#3b82f6] text-[#ffffff] font-poppins cursor-pointer max-lg:text-sm"
          onClick={handleAdd}
        >
          Add Item
        </Button>
